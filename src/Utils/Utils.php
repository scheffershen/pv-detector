...
final class Utils
{
    ...
    public static function no_accent(string $str): string
    {
        $str = htmlentities($str, ENT_NOQUOTES, 'utf-8');
        $str = preg_replace('#&([A-za-z])(?:acute|cedil|caron|circ|grave|orn|ring|slash|th|tilde|uml);#', '\1', $str);
        $str = preg_replace('#&([A-za-z]{2})(?:lig);#', '\1', $str); // pour les ligatures e.g. 'œ'
    ...
    public static function full_text_search(string $keyword, string $text): bool
    {
        if (preg_match("/{$keyword}/i", self::remove_accents($text))) {
            return true;
        } else {
            return false;
        }
    ...
    public static function close_open_html_tags(string $text): string 
    {
        $doc = new \DOMDocument();
    ...
    public static function word_count($text, $dci) {
        $dci = remove_stop_words($dci, 'fr');
        $dci = remove_stop_words($dci, 'en');
        $nb_occurence = 0;
        $words = explode(" ", $dci);
        foreach ($words as $word) {
            if (!empty($text) && !empty($word) && !is_numeric($word))
                $nb_occurence += \substr_count($text, $word);
        }
        return ($nb_occurence > 0)?$nb_occurence:1;
    }

    public static function word_search($text, $dci) {
        $mots = explode("|", $dci);

        $mots_noaccent = array_map(function($mot) {
            return self::clean_text($mot, ['ACCENT', 'MINUSCULE']);
        }, $mots);

        foreach (array_unique($mots_noaccent) as $mot) { 
            $pattern = "/".$mot."/i";
            if ( preg_match($pattern, $text) !== false) return true; 
        }

        return false;
    }

    public static function number_of_words($text, $dci) {
        $nb_occurrence = 0;
        $mots = explode("|", $dci);

        foreach (array_unique($mots) as $mot) { 
            $nb_occurrence += substr_count($text, trim($mot));
        }        
